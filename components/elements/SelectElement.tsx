...
const type: ElementsType = "SelectElement";
...
export const SelectElement: EditorElement = {
    type,
    construct: (id: string) => ({
        id,
        type,
    ...
    function applyChanges() {
        ...
        const createOptions = () => {
            ...
            for (let i = 0; i < opts.length-1; i++){
              optionTag.push(
                `<option key=${i} value='${opts[i]}'>${opts[i]}</option>`
              );
            }
        ...
        const { label, helperText, color, bg, variant, required, width, height, font, weight, opacity, amount, options, code,
...
function DesignerComponent({ elementInstance }: { elementInstance: EditorElementInstance }){
    const element = elementInstance as CustomInstance;
    const { label, helperText, color, bg, variant, required, width, height, font, weight, opacity, amount, options,
