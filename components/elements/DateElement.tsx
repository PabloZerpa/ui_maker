...
const type: ElementsType = "DateElement";
...
export const DateElement: EditorElement = {
    type,
    construct: (id: string) => ({
        id,
        type,
...
function PropertiesComponent({ elementInstance }: { elementInstance: EditorElementInstance }){
    ...
    const labels = ['Label','HelperText','Minimun', 'Maximun'];
    ...
    function applyChanges() {
        ...
        const { label, helperText, color, bg, variant, required, width, height, font, weight, opacity, min, max, code,
                ...
                {labels.map((label, index) => {
                    return(
                        <FormField
                            ...
                            name={label == 'Label' ? 'label' : label == 'HelperText' ? 'helperText' : label == 'Minimun' ? 'min' : 
                                label == "Maximun" ? 'max' : 'label'}
                            render={({ field }) => (
                                <FormItem
...
function DesignerComponent({ elementInstance }: { elementInstance: EditorElementInstance }){
    const element = elementInstance as CustomInstance;
    const { label, helperText, color, bg, variant, required, width, height, font, weight, opacity, min, max,
