...
const type: ElementsType = "TextAreaElement";
...
export const TextAreaElement: EditorElement = {
    type,
    construct: (id: string) => ({
        id,
        type,
...
function PropertiesComponent({ elementInstance }: { elementInstance: EditorElementInstance }){
    ...
    const sizes = ['Width','Height','Minimun','Maximun','Cols','Rows'];
    ...
    function applyChanges() {
        ...
        const { label, helperText, placeHolder, color, bg, variant, required, width, height, font, weight, opacity, minlength, maxlength, cols, rows, code,
                    ...
                    {sizes.map((size, index) => {
                        return(
                            <FormField
                                ...
                                size == "Maximun" ? 'maxlength' : size == "Cols" ? 'cols' : 
                                size == "Rows" ? 'rows' : 'width'}
                                render={({ field }) => (
                                    <FormItem
                                        className="input input-bordered w-full max-w-xs"
                                        onChange={handleChanges}
...
function DesignerComponent({ elementInstance }: { elementInstance: EditorElementInstance }){
    const element = elementInstance as CustomInstance;
    const { label, helperText, placeHolder, color, bg, variant, required, width, height, font, weight, opacity, minlength, maxlength, cols, rows,
